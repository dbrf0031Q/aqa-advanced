  ...
  users.forEach((user) => {
    describe(`Testing registration and login for user: ${user.email}`, () => {
      ...
      it(`should successfully register user: ${user.email}`, () => {
        ...
        cy.get('input[name="password"]').type(user.password, { sensitive: true });
        cy.get('input[name="repeatPassword"]').type(user.password, { sensitive: true });
...
describe('Registration Form Errors Handling', () => {
  ...
  it('should display error message for password mismatch', () => {
    cy.get('input[name="password"]').type('ValidPass123', { sensitive: true });
    cy.get('input[name="repeatPassword"]').type('DifferentPass123', { sensitive: true });
    cy.get('button').contains('Register').click();
